...
Vec2d CatmullRomSpline::operator()(double t_value) const
{
  ...
  const Vec2d lerp01 = interpolate(p0, p1, t0, t1, t_value);
  const Vec2d lerp12 = interpolate(p1, p2, t1, t2, t_value);
  const Vec2d lerp23 = interpolate(p2, p3, t2, t3, t_value);

  // Second level of interpolation
  const Vec2d lerp012 = interpolate(lerp01, lerp12, t0, t2, t_value);
  const Vec2d lerp123 = interpolate(lerp12, lerp23, t1, t2, t_value);

  // Third level of interpolation
  const Vec2d res = interpolate(lerp012, lerp123, t1, t2, t_value);
